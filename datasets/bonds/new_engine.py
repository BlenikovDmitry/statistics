...
#доходность последней сделки должна быть больше нуля и меньше 4-х стандартных отклонений, чтобы отсечь совсем уж жесть
selected_data = ofz[ofz['Дох посл сделки'] > 0]
selected_data = selected_data[selected_data['Дох посл сделки'] < selected_data['Дох посл сделки'].std() * 4].loc[:,['Наименование','Цена % средневзвешенная',
