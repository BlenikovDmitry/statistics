...
#агрегируем все бумаги по непогашенному долгу во всех валютах номинала
#выписываем все наименования во всех валютах номинала и суммируем по непогашенному долгу
selected_data = ofz_set
#selected_data['Непогашенный долг'] = selected_data['Непогашенный долг'].str.replace('-', '0').str.replace(',','.').astype(float)
debt_pivot = selected_data.pivot_table(index = 'Наименование', columns = 'Валюта номинала', values = 'Непогашенный долг', aggfunc = 'sum')
print(debt_pivot)
#агрегируем все бумаги по валюте по наименованию и валюте номинала, выписываем по валюте расчетов и суммируем по непогашенному долгу.
#покажет, какие бумаги, в какоб обЪеме и в какой валюте есть в дата сете
debt_pivot = selected_data.pivot_table(index = ['Наименование','Валюта номинала'], columns = ['Валюта расчетов'], values = 'Непогашенный долг', aggfunc = 'sum')
print(debt_pivot)
